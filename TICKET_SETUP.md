...
The implementation is ready locally. The live project already has the ticketing tables. Apply `003_event_management.sql`, `004_staff_scanning.sql`, `005_ticket_voting.sql`, and `006_deployment_hardening.sql` in that order to finish event settings, open existing sales, enable staff scanning, activate the contest vote, and use deployment-safe upload limits. Ticket notification emails still require the server credentials below. No live database migration or test email was sent during this update.
...
Finally, run `supabase/migrations/006_deployment_hardening.sql`. It aligns the existing receipt and banner buckets with the app's 4 MB upload limit so uploads fit within Vercel's request-body ceiling. All four upgrade migrations are transactional and safe to rerun; reruns preserve votes and admin edits.
