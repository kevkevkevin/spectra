...
export default async function AccountHeader() {
 ...
 return <header className="account-header"><a className="brand" href="/"><img src="/assets/reference/logo-mark.png" alt=""/>SPECTRA</a><nav aria-label="Account navigation"><a href="/contest">Contest</a><a href="/tickets">Tickets</a>{user?<>{admin&&<><a href="/admin/tickets">Admin</a><a href="/admin/voting">Voting</a><a href="/admin/staff">Staff</a></>}{(staff||admin)&&<a href="/staff/scan">Scanner</a>}<a className="button" href="/account">My Account</a><form action={logout}><button className="text-button">Sign out</button></form></>:<a className="button" href="/login">Sign in / Sign up</a>}</nav></header>;
}
