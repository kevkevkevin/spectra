...
export async function GET(request:NextRequest){
 ...
 const round=request.nextUrl.searchParams.get('round')??'elimination';
 if(!UUID.test(campaign)||!isScoringRound(round))return NextResponse.json({error:'Invalid campaign or round'},{status:400,headers:{'Cache-Control':'no-store'}});
 ...
 const db=publicView
  ...
  :await createClient();
 const {data,error}=await db.rpc('contest_judge_leaderboard',{p_campaign_id:campaign,p_round:round});
